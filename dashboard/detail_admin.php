...
<?php
  ...
  $cpassword = $_POST['cpassword'];
    ...
    $sql ="UPDATE `utilisateur` SET `nom`='$nom',`prenom`='$prenom',`telephone`='$telephone',`telephone2`='$telephone2',`email`='$email', `password`='$password' WHERE id=$id";

    $result = mysqli_query($conn, $sql);

    if($result){
      header("Location: list_user.php?msg=Modification reussie ");
    }
    else {
                                ...
                                <div class="form-group row">
                                    <div class="col-sm-6 mb-3 mb-sm-0">
                                        <input type="text" name="telephone" class="form-control form-control-user"
                                            id="exampleInputPassword"  value="<?php echo $row['telephone'] ?>" require>
                                    </div>
                                    <div class="col-sm-6">
                                        <input type="text" name="telephone2" class="form-control form-control-user"
                                            id="exampleRepeatPassword"  value="<?php echo $row['telephone2'] ?>">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <div class="col-sm-6 mb-3 mb-sm-0">
                                        <input type="password" name="password" class="form-control form-control-user"
                                            id="exampleInputPassword"  value="<?php echo $row['password'] ?>" require>
                                    </div>
                                    <div class="col-sm-6">
                                        <input type="password" name="cpassword" class="form-control form-control-user"
                                            id="exampleRepeatPassword"  value="<?php echo $row['password'] ?>" require>
                                    </div>
                                </div>
                                <input  type="submit" name="submit" class="btn btn-primary btn-user btn-block" value="Register" class="btn mb-3" value="Enregistrer">
                                <hr>
